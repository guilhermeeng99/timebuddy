...
void main() {
  setUpAll(() {
    ...
    // source builds from the Google ID token, the provider it hands the web
    // redirect, and the profile it forwards. mocktail matches a fallback by
    // `is T`, so the concrete Google types below cover the `AuthCredential`
    // and `AuthProvider` parameters they are passed as.
  ...
  group('over a mocked data source', () {
    ...
    group('signInWithGoogle', () {
      ...
      test('answers the cancellation marker when nothing happened', () async {
        // Rule 5. No session and no exception is a dismissed dialog, or a
        // redirect that is navigating away: neither is an error.
        when(remote.signInWithGoogle).thenAnswer((_) async => null);
  ...
  group('through the real Firebase data source', () {
    ...
    group('sign-out is platform-specific (rule 6)', () {
      test('skips the Google plugin in a browser', () async {
        // The plugin is never initialised on web — Firebase's redirect flow
        // owns the GSI lifecycle there — so calling into it throws
        // StateError, which is why this branch exists at all.
    ...
    group('sign-in is platform-specific (rule 2)', () {
      test('the browser redirects instead of driving the plugin', () async {
        // A popup trips COOP in several browsers, so web redirects instead.
        when(
        ...
        // The call above navigates away; landing here at all means nothing
        // happened, which is the cancelled outcome rather than an error.
        expect(isSignInCancelled(failureOf(result)), isTrue);
      });

      test('a rejected redirect is a failure, not a cancellation', () async {
        when(() => firebaseAuth.signInWithRedirect(any())).thenThrow(
          FirebaseException(plugin: 'firebase_auth', code: 'popup-blocked'),
        );
    ...
    group('provisioning the profile document (rules 3, 4 and 8)', () {
      ...
      test('the web session comes from the pending redirect', () async {
        ...
        final redirected = _MockUserCredential();
        final redirectedUser = aFirebaseSession(
          creationTime: accountCreatedAt,
        );
        when(() => redirected.user).thenReturn(redirectedUser);
        when(
          firebaseAuth.getRedirectResult,
        ).thenAnswer((_) async => redirected);
        await firestoreHoldsProfile();
